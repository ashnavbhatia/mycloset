class ClothingPiecesController < ApplicationController
  ...
  def index
    ...
    @clothing_pieces = @q.result(:distinct => true).includes(:user, :brand_name, :clothing_type, :borrower).page(params[:page]).per(10)
  end
  ...
  def current_user_must_be_clothing_piece_user
    ...
    def clothing_piece_params
      params.require(:clothing_piece).permit(:user_id, :photo, :size, :brand_name_id, :clothing_type_id, :last_used_date, :borrowed, :borrower_id, :name, :description)
    end
end
